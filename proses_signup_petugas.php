<?php
    ...
    if($_POST){
        ...
        if(!in_array($tipe_foto, $ekstensi)){
			header("location:signup_petugas.php?alert=gagal_ektensi");
		}else{
			if($ukuran < 1044070){			
				move_uploaded_file($tmp, 'tb'.$namafoto);
				...
				if($query){
					echo "<script>alert('Sukses menambahkan user');location.href='login_petugas.php';</script>";
				}else{
					echo "<script>alert('Gagal menambahkan user');location.href='login_petugas.php';</script>";
				}
			}else{
				echo "<script>alert('Ukuran Terlalu Besar');location.href='signup_petugas.php';</script>";
			}
		}
    }
?>